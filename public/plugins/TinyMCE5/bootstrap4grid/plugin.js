...
(function () {
  ...
  function bootstrap4GridPlugin(editor, url) {
    ...
    function Tinymce_bs4_grid() {
      ...
      var parentDOMS = jQuery(node).parents('div.grid');
          ...
          for (i = 0; i < oldGridNumber; i++) {
            ...
            for (j = 0; j < gridClasses.length; j++) {
              if (/^col-.*/i.test(gridClasses[j])) {
                ...
                if (j == 0) {
                  if (gridClasses[0].indexOf('xl') > -1) {
                    screenSize = 'xl';
                  } else if (gridClasses[0].indexOf('lg') > -1) {
                    screenSize = 'lg';
                  } else if (gridClasses[0].indexOf('md') > -1) {
                    screenSize = 'md';
                  } else if (gridClasses[0].indexOf('sm') > -1) {
                    screenSize = 'sm';
                  } else {
                    screenSize = '';
                  }
      ...
      editor.windowManager.open({
        ...
        onSubmit: function (e) {
          ...
          if (editMode) {
            jQuery(parentDOMS).replaceWith(leadingHtml + '<div class="grid ' + 'grid-cols-12">' + htmlContents + '</div>' + endingHtml);
          } else {
            editor.insertContent(leadingHtml + '<div class="grid ' + 'grid-cols-12">' + htmlContents + '</div>' + endingHtml);
          }
          e.close();
        }
      });
