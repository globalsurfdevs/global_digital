...
export async function GET(req: NextRequest) {
  try {
    ...
    const slug = searchParams.get("slug"); // --- Fetch a single service by slug (full data, for the edit form) ---
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const slug = searchParams.get("slug"); // const slug = "branding-and-positioning-agency-in-dubai"

    const body = await req.json(); // Never let the request body overwrite the slug that identifies this item —
    // slug changes should go through a dedicated rename flow, not a general PATCH.
