...
const AdminPortfolio = () => {
  ...
  const getTaskPos = (id: number) =>
    portfolios.findIndex((item: { id: number }) => item.id == id);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            className="rounded-full bg-primary p-2 px-6 text-white"
            onClick={confirmPosition}
          >
            {!reorderMode ? "Reorder" : "Confirm"}
          </button>
          <Link href={"/admin/portfolio/add"}>
            <button className="rounded-full bg-black p-2 px-6 text-white">
              Add new content
            </button>
          </Link>
        </div>
