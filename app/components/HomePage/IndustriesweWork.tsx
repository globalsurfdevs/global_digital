...
interface IndustriesWeWorkProps {
  industry: IndustryProps[];
}

const IndustriesweWork: React.FC<IndustriesWeWorkProps> = ({ industry }) => {
                ...
                const card = (
                  ...
                  <div className="ease-[cubic-bezier(0.25,0.8,0.25,1)] group relative flex h-[300px] flex-col justify-between gap-3 overflow-hidden rounded-sm border border-transparent p-5 transition-all duration-500 hover:-translate-y-1 hover:border-[#E63e31]/40 hover:shadow-2xl hover:shadow-black/20 lg:h-[340px] lg:gap-0 lg:p-10 xl:h-[414px]">
                    ...
                    <Image
                      ...
                      quality={95}
                      loading="lazy"
                      className="duration-[900ms] ease-[cubic-bezier(0.25,0.8,0.25,1)] object-cover grayscale transition-[filter,transform] group-hover:scale-110 group-hover:grayscale-0"
                    />

                    {/* Permanent bottom gradient — text legibility */}
                    <div className="pointer-events-none absolute inset-x-0 bottom-0 h-3/4 bg-gradient-to-t from-black/90 via-black/40 to-transparent transition-opacity duration-700 ease-out group-hover:opacity-90" />

                    {/* Hover darken layer, on top of the gradient */}
                    <div className="ease-[cubic-bezier(0.25,0.8,0.25,1)] pointer-events-none absolute inset-0 bg-black/0 transition-colors duration-700 group-hover:bg-black/30" />

                    {/* Content */}
                    <div className="relative z-10 flex h-full flex-col justify-end">
                      <div className="ease-[cubic-bezier(0.25,0.8,0.25,1)] p-0 transition-transform duration-500 group-hover:-translate-y-1 md:pt-[30px]">
                        <h3 className="text-30 titlesp text-[#E63e31] transition-colors duration-300 group-hover:text-white">
                          {item.name}
                        </h3>
