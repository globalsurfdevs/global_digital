...
import { MenuItem } from "./MenuItemv2-opt";
...
export const Navigation: React.FC<MenuToggleProps> = ({
  ...
  return (
    <div className="">
      {modalOpen && (
        <div className="top-13 fixed left-0 z-[1000]  w-screen overflow-y-auto bg-white lg:top-0 ">
          <LetsTalk
            onClose={() => {
              setModalOpen(false);
              onHide();
      ...
      <motion.ul className="mobile-menu-ul mb-5">
        {menuItems.map((menuItem, index) => (
          // MenuItem already renders its own <li> (w-full); a wrapping <li>
          // here would nest <li> in <li>, which breaks hydration now that the
          // menu is server-rendered.
              ...
              {menuItem.children &&
                menuItem.children.map((child, childIndex) => {
                  ...
                  return (
                    <li
                      key={childIndex}
                      className="flex w-full flex-col items-start border-b last:border-none"
                    >
                      {/* Pillar name navigates to its page; only the arrow
                          expands/collapses its services. */}
                      {/* Row padding lives on the link so the whole row height
                          navigates; only the arrow button toggles. */}
                      <div className="flex w-full cursor-pointer items-center">
                        <Link
                          href={child.url}
                          className="flex flex-1 items-center gap-2 py-4 text-sm uppercase"
                          onClick={() => {
                            toggle(); // ✅ Close menu
                          }}
                        >
                        ...
                        {hasSubmenu && (
                          <button
                            type="button"
                            // p-2 enlarges the tap area; -my-2 keeps the row
                            // height and no right margin keeps the arrow where
                            // the old `mr-2` span placed it.
                            className="-my-2 p-2"
                            aria-label={`Show ${child.item} services`}
                            aria-expanded={!!openSubmenus[submenuKey]}
                            onClick={(e) => {
                              e.stopPropagation();
                      ...
                      {hasSubmenu && (
                        <motion.ul
                          ...
                          initial="hidden"
                          animate={
                            openSubmenus[submenuKey] ? "visible" : "hidden"
                          }
                          exit="exit"
                        >
                            ...
                            return (
                              <li key={subChildIndex} className="flex flex-col">
                                ...
                                >
                                  <Link
                                    href={subChild.url}
                                    className={`flex flex-1 items-center gap-2 text-sm uppercase no-underline ${subChildIndex === (child.children?.length ?? 0) - 1 ? "pb-4" : ""}`}
                                    onClick={(e) => {
                                      if (hasSubSubmenu) {
                                        e.preventDefault();
                                      } else {
                                  ...
                                  >
                                    <div className="size-[6px] bg-primary"></div>
                                    {subChild.svg && (
                                      <Image
                                        src={subChild.svg}
                                        alt={subChild.item}
                                    ...
                                    {subChild.children?.map(
                                      (subSubChild, subSubChildIndex) => (
                                        <li key={subSubChildIndex}>
                                          <Link
                                            href={subSubChild.url}
                                            className="flex items-center gap-2 px-2 py-1 text-xs hover:underline"
                                          >
                                            {subSubChild.svg && (
                                              <Image
                                                src={subSubChild.svg}
