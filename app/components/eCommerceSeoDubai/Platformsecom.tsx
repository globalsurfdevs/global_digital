...
const Platformsecom: React.FC<PlatformsSectionProps> = ({
  ...
  return (
    <div className={`container mx-auto py-4  ${leftzero ? "relative" : ""}`}>
      <div
        className={`mb-[50px] flex flex-col gap-7 lg:mb-[140px] ${bgblack ? "bg-black" : "bg-dgray"} ${leftzero ? "left0w" : ""} p-[25px] lg:gap-12 lg:p-[100px]`}
      >
        <div>
          <motion.div
            initial="hidden"
          ...
          >
            ...
            <p className="text-19 fnt-lexend max-w-[76ch] text-gray1 transition-colors duration-300 group-hover:text-gray-700 ">
              {desc}{" "}
            </p>
          </motion.div>
        </div>
        ...
        >
          <div
            className={`grid grid-cols-1 gap-x-8 gap-y-[45px] md:grid-cols-2 lg:gap-y-[95px]  ${colm ? `xl:grid-cols-${colm}` : "xl:grid-cols-4"}`}
          >
            {data.map((platform) => (
              <div
                key={platform.id}
          ...
          {btmpara && (
            <p className="text-19 fnt-lexend md:pt:6 mb-0 max-w-[76ch] pt-4 text-gray1 transition-colors duration-300 group-hover:text-gray-700 xl:pt-[60px]">
              {btmpara}{" "}
            </p>
          )}
        </motion.div>
